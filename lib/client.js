window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var HostServices = { remoteSession: null, conversation: null, sessions: null };

		/**
		 * Only ordinary, catalogued Sessions may expose history. A direct
		 * subagent has a different durable address and must never be queried with
		 * the ordinary session address. Unknown or not-yet-loaded identities stay
		 * closed until the Sessions service can prove they are ordinary.
		 */
		function historyAllowed(sessionId) {
			if (!sessionId || !HostServices.sessions) return false;
			var sessions = HostServices.sessions;
			if (typeof sessions.subagentAddress !== "function" || !sessions.list || typeof sessions.list.getSnapshot !== "function") return false;
			try {
				if (sessions.subagentAddress(sessionId) != null) return false;
				var list = sessions.list.getSnapshot();
				if (!list || list.phase !== "ready" || !list.byId) return false;
				var row = list.byId[sessionId];
				return !!row && row.origin !== "subagent";
			} catch (_error) {
				return false; // identity lookup failure is fail-closed
			}
		}
		...
		function loadSessionRows(sessionId) {
			if (!historyAllowed(sessionId)) return;
			...
			pageAllUserMessages(rs, sessionId).then(function (items) {
				if (!historyAllowed(sessionId)) {
					if (HistoryStore.get().sessionId === sessionId) HistoryStore.set({ open: false, index: 0, savedDraft: "", mode: "replace", prefix: "", suffix: "", sessionId: null, phase: "idle", rows: [] });
					return;
				}
		...
		function loadFullHistory(sessionId, cb) {
			if (!historyAllowed(sessionId)) { cb([]); return; }
			var rs = HostServices.remoteSession;
		...
		function finish(sid, entry, rows, err) {
			...
			if (!err && !historyAllowed(sid)) {
				rows = [];
				err = new Error("history is unavailable for this session");
			}
		...
		function walkHistory(dir) {
			...
			if (!sid || ArrowWalk.waiting || !historyAllowed(sid)) return;
			...
			loadFullHistory(sid, function (loaded, err) {
				...
				if (!historyAllowed(sid)) { exitArrowWalk(); return; }
				if (err) {
		...
		function HistoryOverlay(props) {
			...
			var allowed = historyAllowed(sessionId);
			var open = allowed && store.open && store.sessionId === sessionId;
			var index = store.index;
			var rows = open ? store.rows : [];
			var status = open ? store.phase : "idle";
			var activeSession = allowed ? sessionId : null;
			...
			function applyItem(item) {
				if (!activeSession || !historyAllowed(activeSession) || !item) return;
				var snapshot = HistoryStore.get();
			...
			return react.createElement("div", {
				...
				react.createElement("input", {
					...
					// type=search (not text) keeps Bitwarden/password managers from
					// treating this panel's filter box as a form field and popping
					// their own inline autofill menu (its ignoredInputTypes excludes "search").
					type: "search",
		...
		function apply(ctx) {
			...
			ctx.inject(["remote", "remote.session", "conversation", "sessions"], function (hctx) {
				HostServices.remoteSession = hctx.get("remote.session");
				HostServices.conversation = hctx.get("conversation");
				HostServices.sessions = hctx.get("sessions");
			});
				...
				sctx.effect(function () {
					return triggers.registerSource({
						...
						candidates: function (session, req) {
							var sid = session ? session.sessionId : null;
							if (!historyAllowed(sid)) return Promise.resolve([]);
						...
						onPick: function (pick) {
							...
							if (!historyAllowed(sid)) return void 0;
