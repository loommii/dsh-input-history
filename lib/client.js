window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var PKG_NAME = "@linjjj/dsh-input-history";
		...
		var LANG = (typeof document !== "undefined" && document.documentElement && (document.documentElement.lang || "")).toLowerCase().indexOf("zh") === 0 ? "zh" : "en";
		var STR = {
			zh: {
				empty: "暂无历史记录", loading: "正在加载…",
				cmdName: "history", cmdDesc: "浏览本会话发送过的消息并选回输入框",
				searchPh: "搜索…", panelAria: "输入历史列表"
			},
			en: {
				empty: "No history yet", loading: "Loading…",
		...
		async function pageAllUserMessages(remoteSession, sessionId, signal) {
			...
			while (guard++ < 300) {
				...
				if (firstPage) {
					...
					if (!result || result.ok !== true || !result.value) break;
		...
		/** Single-flight full-history loader callbacks (panel + walk share one load). */
		var HistoryLoad = { sessionId: null, inflight: false, cbs: [] };
		...
		/** Load the full log for a session into the store. */
		function loadSessionRows(sessionId) {
			...
			pageAllUserMessages(rs, sessionId, controller.signal).then(function (items) {
				...
				HistoryStore.set({ phase: "ready", rows: out, index: 0 });
			}, function () {
				if (HistoryStore.get().sessionId !== sessionId) return;
				HistoryStore.set({ phase: "empty", rows: [] });
			});
		...
		function loadFullHistory(sessionId, cb) {
			...
			if (HistoryLoad.sessionId === sessionId && HistoryLoad.inflight) {
				HistoryLoad.cbs.push(cb);
				return;
			}
			var rs = HostServices.remoteSession;
			if (!sessionId || !rs || typeof rs.follow !== "function" || typeof rs.page !== "function") {
			...
			HistoryLoad.sessionId = sessionId;
			HistoryLoad.inflight = true;
			HistoryLoad.cbs = [cb];
			var controller = new AbortController();
			pageAllUserMessages(rs, sessionId, controller.signal).then(function (items) {
				var rows = [];
				...
				finish(sessionId, rows);
			}, function () {
				finish(sessionId, []);
			});
			function finish(sid, rows) {
				if (HistoryLoad.sessionId !== sid) return;
				HistoryLoad.sessionId = null;
				HistoryLoad.inflight = false;
				var cbs = HistoryLoad.cbs;
				HistoryLoad.cbs = [];
				if (HistoryCache.sessionId === sid) HistoryCache.rows = rows;
				for (var j = 0; j < cbs.length; j++) cbs[j](rows);
			}
		}
		...
		function openHistoryPanel(sessionId, opts) {
			...
			loadFullHistory(sessionId, function (rows) {
		...
		function walkHistory(dir) {
			...
			loadFullHistory(sid, function (loaded) {
		...
		function startArrowWalk(sessionId) {
			...
			ArrowWalk.waiting = false;
			ArrowWalk.rows = [];
			HistoryCache.sessionId = sessionId; // warm the shared cache for repeats
			walkHistory(-1);
		}
		...
		var inject = ["slots", "commandUi", "inputTriggers"];
